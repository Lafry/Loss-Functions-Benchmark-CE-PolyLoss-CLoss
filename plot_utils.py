...
def plot_training_history(history: dict, model_name: str = "Modello", gdv_iniziale: float = None, save_pdf: bool = True, dataset_id: int = None, base_dir: str = "results"):
    """
    ...
    Salva i file in formato PDF con timestamp nella struttura: results/<dataset_id>/plots/
    """
    ...
    if save_pdf:
        # Se non viene passato un ID, usa una cartella generica
        folder_id = str(dataset_id) if dataset_id is not None else "generic"
        
        # Costruisce il percorso: results/365/plots
    ...
    if save_pdf:
        filepath = os.path.join(percorso_cartella, f"{base_filename}_Loss.pdf")
    ...
    if save_pdf:
        filepath = os.path.join(percorso_cartella, f"{base_filename}_Accuracy.pdf")
        plt.savefig(filepath, format='pdf', bbox_inches='tight')
        print(f"[*] Grafico salvato: {filepath}")
    ...
    if ha_gdv:
        ...
        if save_pdf:
            filepath = os.path.join(percorso_cartella, f"{base_filename}_GDV.pdf")
            plt.savefig(filepath, format='pdf', bbox_inches='tight')
            print(f"[*] Grafico salvato: {filepath}")
