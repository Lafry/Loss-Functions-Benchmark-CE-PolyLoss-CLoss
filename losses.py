...
class CombinedCLoss(nn.Module):
	...
	"""
	def __init__(self, sigma: float = 0.5, gamma: float = 0.5, reduction: str = 'mean'):
	...
	def forward(self, outputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
		# Calcolo della Cross-Entropy base
		ce_loss = self.ce(outputs, targets)
		
		# Calcolo della C-Loss (Correntropy-induced) basata sull'errore CE
		c_loss = 1.0 - torch.exp(- (ce_loss ** 2) / (2 * self.sigma ** 2))
		
		# Combinazione convessa delle due Loss
